...
References: [Draft 2 Section 5.20](https://json-schema.org/draft-02/draft-zyp-json-schema-02.txt), [Draft 3 Section 5.23](https://json-schema.org/draft-02/draft-zyp-json-schema-02.txt)

> Behaviour:   
      >>[Before](https://json-schema.org/draft-02/draft-zyp-json-schema-02.txt): Not required to validate  
        [After](https://json-schema.org/draft-02/draft-zyp-json-schema-02.txt): MAY validate (but aren’t required to)
...
References: [Draft 3 Section 5.23](https://json-schema.org/draft-02/draft-zyp-json-schema-02.txt), [Draft 4](https://json-schema.org/draft-04/draft-fge-json-schema-validation-00#rfc.section.7
)

> Behaviour:  
      >>[Before](https://json-schema.org/draft-02/draft-zyp-json-schema-02.txt): MAY validate.   
...
| Change Type | Format Attribute | Description                                                                                                          |
|-------------|------------------|----------------------------------------------------------------------------------------------------------------------|
| Added       | `uri-reference`    | This attribute applies to string instances. A string instance is valid against this attribute if it is a valid URI Reference (either a URI or a relative-reference), according to [RFC3986] |
| Added       | `uri-template`     | This attribute applies to string instances. A string instance is valid against this attribute if it is a valid URI Template (of any level), according to [RFC6570]                          |
| Added       | `json-pointer`     | This attribute applies to string instances. A string instance is valid against this attribute if it is a valid JSON Pointer, according to [RFC6901]                                            |
...
| Added       | `idn-email`                  | As defined by RFC 6531 [RFC6531] |
| | |idn-email is a superset of email              |
| Added       | `hostname`                   | As defined by RFC 1034, section 3.1 [RFC1034], including host names produced using the Punycode algorithm specified in RFC 5891, section 4.4 [RFC5891] |
| Added       | `idn-hostname`               | As defined by either RFC 1034 as for hostname, or an internationalized hostname as defined by RFC 5890, section 2.3.2.3 [RFC5890]  |
|||idn-hostname is a superset of hostname|
| Added       | `ipv6`                       | A string instance is valid against this attribute if it is a valid representation of an IPv6 address as defined in RFC 2373, section 2.2 [RFC2373] An IPv6 address as defined in RFC 4291, section 2.2 [RFC4291] |
| Added       | `iri`                        | A string instance is valid against this attribute if it is a valid IRI according to [RFC3987]                                                           |
| Added       | `iri-reference`              | A string instance is valid against this attribute if it is a valid IRI Reference (either an IRI or a relative-reference) according to [RFC3987]  |
|||iri is a superset of uri. iri-reference is a superset of uri-reference|
| Added       | `relative-json-pointer`      | A string instance is valid against this attribute if it is a valid Relative JSON Pointer [relative-json-pointer]                                          |
| Added       | `regex`                      | A regular expression, which SHOULD be valid according to the ECMA 262 [ecma262] regular expression dialect                                               |
...
| Added       | `uuid`             | A string instance is valid against this attribute if it is a valid string representation of a UUID, according to [RFC4122] |

### Changes :

| Keyword   | Previous Description | Present Description |
|-----------|-----------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------|
